...
class TestAppium(unittest.TestCase):
    ...
    def test_search(self):
        ...
        #price: low to high
        lowfirst_field = self.driver.find_element(
            AppiumBy.XPATH, "//android.widget.CheckBox[@resource-id='sort/price-asc-rank']"
        )
        lowfirst_field.click()
        print("Clicked on ascending filter")
        ...
        print("Lowest Priced Headphone: \n")
        print(f"Name: {product_name}")
        print(f"Price: {price}")

        product_time = time.time() - start_time

        total_time = homepage_load_time + search_time + filter_time + product_time

        #part e and f
        f = open("partf.txt", "a")
        e = open("parte.txt", "a")

        f.write(f"Amazon homepage load time(Part B): {homepage_load_time:.2f} seconds\n")
        f.write(f"Amazon homepage Effective(excluding Sleep time) load time(Part B): {homepage_load_time-6:.2f} seconds\n")
        f.write("*********************************************************************************\n")

        e.write(f"Activity1: homepage load time(Part B): {homepage_load_time:.2f} seconds\n")
        e.write(f"Activity1: Effective(excluding Sleep time) homepage load time(Part B): {homepage_load_time-6:.2f} seconds\n")
        e.write(f"Activity2: Search Time(Part B): {search_time:.2f} seconds\n")
        e.write(f"Activity2: Effective(excluding Sleep time) Search time(Part B): {search_time-9:.2f} seconds\n")
        e.write(f"Activity3: filter time(Part B): {filter_time:.2f} seconds\n")
        e.write(f"Activity3: Effective(excluding Sleep time) filter time(Part B): {filter_time-10:.2f} seconds\n")
        e.write(f"Activity4: product retrieval time(Part B): {product_time:.2f} seconds\n")
        e.write(f"Activity4: Effective(excluding Sleep time) product retrieval time(Part B): {product_time-2:.2f} seconds\n")
        e.write(f"Total Time Taken: {total_time:.2f} seconds\n")
        e.write(f"Effective(excluding Sleep time) Total Time Taken: {total_time-27:.2f} seconds\n")
        e.write("*********************************************************************************\n")
